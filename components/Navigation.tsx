...
export default function Navigation({ currentPage, setCurrentPage }: NavigationProps) {
  ...
  const menuItems = [
    ...
    { id: "threads", label: "Stories", icon: "📖" },
    { id: "stories", label: "AURA Stories", icon: "✨" },
    ...
    <>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          {/* Mobile menu panel */}
          <div className="fixed top-0 right-0 bottom-0 w-64 glass-effect border-l border-gray-800 transform transition-transform duration-300 ease-in-out">
                ...
                {menuItems.map((item) => (
                  <button
                    ...
                    className={`w-full flex items-center px-4 py-3 rounded-lg text-left font-medium transition-all duration-300 ${
                      currentPage === item.id
                        ? "premium-gradient text-white border-l-4 border-blue-400"
                        : "text-gray-300 hover:text-white hover:bg-gray-700"
                    }`}
                  >
